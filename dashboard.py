...
with tab[0]:
    ...
    # Website sessions and orders by month
...
with tab[1]:
    ...
    # Total Pageviews, Sessions & Orders
    total_pageviews = website_pageviews.shape[0]
    total_sessions = website_sessions['website_session_id'].nunique()
    total_orders = orders['order_id'].nunique()

    # Conversion, Dropoff and Retention Rates
    conversion_rate = total_orders / total_sessions
...
with tab[2]:
    ...
    #TOTAL UNITS SOLD
    product_sales = order_items.groupby("product_id").size().reset_index(
    ...
    order_item_refunds['created_at'] = pd.to_datetime(
    ...
    refund_trend = order_item_refunds.groupby('month')['refund_amount_usd'].sum().reset_index(
        name='refund_amount'
    )

    trend = revenue_trend.merge(orders_trend, on='month', how='left').merge(
        refund_trend,
        on='month',
        how='left'
    ).fillna(0)
    ...
    products_refund = products_refund.merge(
        products[["product_id", "product_name"]],
        on="product_id",
        how="left"
    )
...
with tab[3]:
    ...
    #KPI Metrics Calculation: New Users
    ...
    #KPI Metrics Calculation: Average Order Gap (Days)
    ...
    #KPI Metrica Calculation: Repeat Customer AOV
    ...
    #KPI Metrics Calculation: Repeat Customer Percentage
    ...
    total_customers = user_product.groupby(
        "product_name"
    ...
    result["repeat_purchase_rate_%"] = (
        result["repeat_customers"] / result["total_customers"]
    ) * 100
    ...
    # Dashboard Orientation
    tab3_columns = st.columns([0.55,0.45],gap='large')
    with tab3_columns[0]:
    ...
    with tab3_columns[1]:
        st.plotly_chart(
            vis.plot_repeat_purchase_rate(result),
            use_container_width=True
        )

        st.plotly_chart(
            vis.create_user_pie_chart(user_counts),
            use_container_width=True,
        )
